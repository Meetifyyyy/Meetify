function getCoverImage(theme) {
  const covers = {
    design: 'https://images.unsplash.com/photo-1561070791-2526d30994b5?w=1200&h=400&fit=crop',
    ai: 'https://images.unsplash.com/photo-1677442136019-21780ecad995?w=1200&h=400&fit=crop',
    startup: 'https://images.unsplash.com/photo-1559136555-9303baea8ebd?w=1200&h=400&fit=crop',
    hackathon: 'https://images.unsplash.com/photo-1504384308090-c894fdcc538d?w=1200&h=400&fit=crop',
    gaming: 'https://images.unsplash.com/photo-1542751371-adc38448a05e?w=1200&h=400&fit=crop',
    coding: 'https://images.unsplash.com/photo-1461749280684-dccba630e2f6?w=1200&h=400&fit=crop',
    sports: 'https://images.unsplash.com/photo-1461896836934-bd45ba8fcf9b?w=1200&h=400&fit=crop',
    default: 'https://images.unsplash.com/photo-1522071820081-009f0129c71c?w=1200&h=400&fit=crop',
  };
  return covers[theme] || covers.default;
}

export const communities = {
  design: {
    id: 'design',
    name: 'Design Buddies',
    avatar: 'D',
    color: 'linear-gradient(135deg, #EC4899, #F97316)',
    desc: 'A community for designers of all levels. Share your work, get feedback, and stay inspired.',
    members: 12400,
    online: 342,
    created: 'March 2024',
    categories: ['art', 'design'],
    tagColor: '#EC4899',
    coverTheme: 'design',
    coverImage: getCoverImage('design'),
    discussionsToday: 42,
    eventsThisMonth: 3,
    newMembersThisWeek: 156,
    trending: false,
    growth: '+156 this week',
    categoryLabel: 'Design',
    ranking: 3,
    memberAvatars: ['E', 'D', 'S', 'R', 'N'],
    interests: ['UI/UX', 'Figma', 'Brand Identity', 'Visual Design', 'Typography', 'Design Systems'],
    rules: ['Share weekly design challenges', 'Build a portfolio review program', 'Host monthly design talks with industry pros'],
    topContributors: [
      { name: 'Emma L.', avatar: 'E', contributions: 89 },
      { name: 'David K.', avatar: 'D', contributions: 64 },
      { name: 'Sophie M.', avatar: 'S', contributions: 52 },
    ],
    memberList: [
      { name: 'Emma L.', avatar: 'E', role: 'UI/UX Designer', admin: true, badge: 'Design Lead' },
      { name: 'David K.', avatar: 'D', role: 'Product Designer', badge: 'Top Creator' },
      { name: 'Sophie M.', avatar: 'S', role: 'Visual Designer' },
      { name: 'Ryan T.', avatar: 'R', role: 'Design Lead', badge: 'Moderator' },
      { name: 'Nina P.', avatar: 'N', role: 'Freelance Designer' },
      { name: 'Oscar V.', avatar: 'O', role: 'Motion Designer' },
      { name: 'Lena Q.', avatar: 'L', role: 'Brand Designer' },
    ],
    posts: [
      { id: 'c_design_1', authorId: 'u1', time: '1 hour ago', text: 'Just redesigned my portfolio using glassmorphism — love how this trend is evolving! Thoughts on neumorphism vs glass? ✨', likes: 31, comments: 14 },
      { id: 'c_design_2', authorId: 'u6', time: '3 hours ago', text: "Anyone here using Figma plugins for auto-layout? Just discovered \"Autoflow\" and it's a game changer for wireframing 🎨", likes: 18, comments: 7 },
      { id: 'c_design_3', authorId: 'u4', time: '6 hours ago', text: "Weekly design challenge: redesign a music player app. Here's my take on a minimal Spotify concept. Feedback welcome!", likes: 45, comments: 22 },
      { id: 'c_design_4', authorId: 'u2', time: '1 day ago', text: "Is anyone here attending the UX summit next week? Let's connect!", likes: 25, comments: 4 },
      { id: 'c_design_5', authorId: 'u7', time: '2 days ago', text: 'Does anyone have good resources for learning motion design in UI? I feel like my interfaces are too static.', likes: 45, comments: 12 },
    ],
  },
  aiml: {
    id: 'aiml',
    name: 'AI/ML Enthusiasts',
    avatar: 'A',
    color: 'linear-gradient(135deg, #3B82F6, #06B6D4)',
    desc: 'Dive into artificial intelligence and machine learning. From beginner tutorials to cutting-edge research.',
    members: 8200,
    online: 215,
    created: 'January 2024',
    categories: ['technology', 'science'],
    tagColor: '#3B82F6',
    coverTheme: 'ai',
    coverImage: getCoverImage('ai'),
    discussionsToday: 38,
    eventsThisMonth: 3,
    newMembersThisWeek: 203,
    trending: false,
    growth: '+203 this week',
    categoryLabel: 'AI',
    ranking: 2,
    memberAvatars: ['R', 'M', 'A', 'K', 'L'],
    interests: ['Deep Learning', 'NLP', 'Computer Vision', 'MLOps', 'Reinforcement Learning', 'Data Engineering'],
    rules: ['Run weekly paper reading sessions', 'Build open-source ML projects together', 'Host quarterly AI hackathons'],
    topContributors: [
      { name: 'Rahul V.', avatar: 'R', contributions: 120 },
      { name: 'Dr. Kim', avatar: 'K', contributions: 95 },
      { name: 'Mia W.', avatar: 'M', contributions: 78 },
    ],
    memberList: [
      { name: 'Rahul V.', avatar: 'R', role: 'ML Engineer', admin: true, badge: 'AI Expert' },
      { name: 'Mia W.', avatar: 'M', role: 'Data Scientist', badge: 'Top Contributor' },
      { name: 'Alex J.', avatar: 'A', role: 'ML Beginner' },
      { name: 'Dr. Kim', avatar: 'K', role: 'AI Researcher', badge: 'Research Lead' },
      { name: 'Leo Z.', avatar: 'L', role: 'NLP Engineer' },
      { name: 'Ivy C.', avatar: 'I', role: 'Computer Vision' },
      { name: 'Ethan G.', avatar: 'E', role: 'MLOps' },
    ],
    posts: [
      { id: 'c_aiml_1', authorId: 'u3', time: '2 hours ago', text: 'Fine-tuned Llama 3 on my own dataset this weekend — the results are incredible. Open-sourcing the notebook soon! 🤖', likes: 52, comments: 18 },
      { id: 'c_aiml_2', authorId: 'u5', time: '5 hours ago', text: 'Can we talk about RAG architectures? Just implemented a hybrid search pipeline with dense + sparse retrievers. Huge improvement over vanilla semantic search.', likes: 27, comments: 11 },
      { id: 'c_aiml_3', authorId: 'u1', time: '8 hours ago', text: 'New to ML and feeling overwhelmed. Any recommendations for structured learning paths that actually work?', likes: 33, comments: 26 },
      { id: 'c_aiml_4', authorId: 'u8', time: '1 day ago', text: 'Rust + ML? Anyone exploring burn.rs framework?', likes: 65, comments: 12 },
      { id: 'c_aiml_5', authorId: 'u4', time: '3 days ago', text: 'Has anyone here used the new Claude 3.5 Sonnet API yet? The reasoning capabilities are off the charts.', likes: 110, comments: 45 },
    ],
  },
  startup: {
    id: 'startup',
    name: 'Startup Hub',
    avatar: 'S',
    color: 'linear-gradient(135deg, #22C55E, #10B981)',
    desc: 'Connect with founders, builders, and investors. Share your journey, find co-founders, and build the future.',
    members: 6700,
    online: 189,
    created: 'June 2024',
    categories: ['business', 'technology'],
    tagColor: '#22C55E',
    coverTheme: 'startup',
    discussionsToday: 56,
    eventsThisWeek: 15,
    newMembersThisWeek: 342,
    trending: false,
    growth: '+342 this week',
    categoryLabel: 'Startup',
    memberAvatars: ['J', 'T', 'C', 'M', 'A'],
    memberList: [
      { name: 'Jordan B.', avatar: 'J', role: 'Founder', admin: true },
      { name: 'Taylor R.', avatar: 'T', role: 'Product Manager' },
      { name: 'Casey L.', avatar: 'C', role: 'Solo Founder' },
      { name: 'Morgan S.', avatar: 'M', role: 'VC Analyst' },
      { name: 'Avery D.', avatar: 'A', role: 'Growth Hacker' },
      { name: 'Reese W.', avatar: 'R', role: 'Angel Investor' },
      { name: 'Drew H.', avatar: 'D', role: 'CTO' },
    ],
    posts: [
      { id: 'c_startup_1', authorId: 'u1', time: '1 hour ago', text: 'Launched our MVP today! A micro-SaaS for remote team standups. Bootstrapped and proud. Would love some beta testers 🚀', likes: 67, comments: 23 },
      { id: 'c_startup_2', authorId: 'u6', time: '4 hours ago', text: "Hot take: founder-market fit matters more than product-market fit in early stages. What's your take?", likes: 41, comments: 19 },
      { id: 'c_startup_3', authorId: 'u2', time: '9 hours ago', text: 'Looking for a technical co-founder (full-stack, React/Node) for my ed-tech startup. Have 2k waitlist users already. DM me!', likes: 29, comments: 15 },
      { id: 'c_startup_4', authorId: 'u8', time: '12 hours ago', text: 'Just hit $1k MRR after 4 months of building! Never give up!', likes: 120, comments: 34 },
      { id: 'c_startup_5', authorId: 'u5', time: '2 days ago', text: 'Is YC still worth it in 2024? Debating whether to apply for the next batch or just stay bootstrapped.', likes: 88, comments: 56 },
    ],
  },
  hackathon: {
    id: 'hackathon',
    name: 'Hackathon Heroes',
    avatar: 'H',
    color: 'linear-gradient(135deg, #A855F7, #EC4899)',
    desc: 'The ultimate community for hackathon enthusiasts. Find teams, celebrate wins, and prep for your next buildathon.',
    members: 4100,
    online: 127,
    created: 'August 2024',
    categories: ['technology', 'gaming'],
    tagColor: '#A855F7',
    coverTheme: 'hackathon',
    discussionsToday: 29,
    eventsThisWeek: 6,
    newMembersThisWeek: 98,
    trending: false,
    growth: '+98 this week',
    categoryLabel: 'Hackathons',
    memberAvatars: ['J', 'S', 'R', 'Q', 'B'],
    memberList: [
      { name: 'Jamie N.', avatar: 'J', role: 'Full-stack Dev', admin: true },
      { name: 'Sam W.', avatar: 'S', role: 'Hackathon Vet' },
      { name: 'Riley P.', avatar: 'R', role: 'Organizer' },
      { name: 'Quinn H.', avatar: 'Q', role: 'Mobile Dev' },
      { name: 'Blake C.', avatar: 'B', role: 'Hardware Hacker' },
      { name: 'Skyler M.', avatar: 'S', role: 'DevOps' },
      { name: 'Casey N.', avatar: 'C', role: 'UI Designer' },
    ],
    posts: [
      { id: 'c_hackathon_1', authorId: 'u7', time: '30 min ago', text: '48-hour hackathon done! We built an AI-powered study buddy using Whisper + GPT-4o. 2nd place overall! 🏆', likes: 89, comments: 31 },
      { id: 'c_hackathon_2', authorId: 'u4', time: '3 hours ago', text: "Tips for first-timers at hackathons: 1) Sleep is optional 2) Pizza is fuel 3) Don't over-scope — build the demo, not the production app", likes: 56, comments: 12 },
      { id: 'c_hackathon_3', authorId: 'u3', time: '7 hours ago', text: 'Looking for team members for the upcoming ClimateTech Hackathon in SF. Need a designer and a backend dev!', likes: 22, comments: 9 },
      { id: 'c_hackathon_4', authorId: 'u5', time: '1 day ago', text: 'Hardware hackathons are so much more stressful than software ones. Change my mind.', likes: 45, comments: 18 },
      { id: 'c_hackathon_5', authorId: 'u1', time: '3 days ago', text: 'My team just won the track prize at HackMIT! So exhausted but so happy.', likes: 320, comments: 45 },
    ],
  },
  gaming: {
    id: 'gaming',
    name: 'Gaming & E-Sports',
    avatar: 'G',
    color: 'linear-gradient(135deg, #F43F5E, #FB923C)',
    desc: 'Connect with gamers from around the world. Find teammates and stay updated on e-sports news.',
    members: 15400,
    online: 890,
    created: 'September 2024',
    categories: ['gaming', 'technology'],
    tagColor: '#F43F5E',
    coverTheme: 'gaming',
    discussionsToday: 67,
    eventsThisWeek: 4,
    newMembersThisWeek: 412,
    trending: false,
    growth: '+412 this week',
    categoryLabel: 'Gaming',
    memberAvatars: ['A', 'J', 'C', 'E', 'M'],
    memberList: [
      { name: 'Alex M.', avatar: 'A', role: 'Streamer', admin: true },
      { name: 'Johnson W.', avatar: 'J', role: 'Game Developer' },
      { name: 'Chris P.', avatar: 'C', role: 'Pro Player' },
      { name: 'Emma T.', avatar: 'E', role: 'Casual Gamer' },
    ],
    posts: [
      { id: 'c_gaming_1', authorId: 'u7', time: '1 hour ago', text: 'Has anyone tried the new Unreal Engine 5.4 features? The performance improvements are insane!', likes: 120, comments: 45 },
      { id: 'c_gaming_2', authorId: 'u4', time: '4 hours ago', text: 'Going live in 10 minutes! We are grinding ranked today. Come hang out in the chat! 🎮', likes: 85, comments: 12 },
      { id: 'c_gaming_3', authorId: 'u8', time: '8 hours ago', text: 'Anyone want to group up for some casual matches tonight? I main support.', likes: 15, comments: 8 },
      { id: 'c_gaming_4', authorId: 'u2', time: '1 day ago', text: 'Just beat Elden Ring for the first time. What a masterpiece. What game should I play next?', likes: 200, comments: 65 },
      { id: 'c_gaming_5', authorId: 'u6', time: '2 days ago', text: 'Looking to form a team for the upcoming Valorant community tournament. Must be Ascendant+.', likes: 30, comments: 20 },
    ]
  },
  gla: {
    id: 'gla',
    name: 'GLA University',
    avatar: '/images/gla-logo.png',
    color: 'linear-gradient(135deg, #1D4ED8, #3B82F6)',
    desc: 'The official community for GLA University students. Connect with batchmates and stay updated on campus events.',
    members: 4200,
    online: 854,
    established: '1998',
    location: 'Mathura, Uttar Pradesh, India',
    website: 'https://www.gla.ac.in',
    collegeType: 'Deemed University',
    affiliatedUniversity: 'UGC Recognized',
    contactEmail: 'info@gla.ac.in',
    departments: [
      { id: 'dep_cs', name: 'Computer Science', icon: '💻', members: 1240, communityId: 'gla_coding' },
      { id: 'dep_sports', name: 'Sports Club', icon: '⚽', members: 850, communityId: 'gla_sports' },
      { id: 'dep_music', name: 'Music & Arts', icon: '🎵', members: 430, communityId: null },
      { id: 'dep_lit', name: 'Literary Club', icon: '📚', members: 360, communityId: null },
      { id: 'dep_robotics', name: 'Robotics Club', icon: '🤖', members: 290, communityId: null },
      { id: 'dep_photo', name: 'Photography', icon: '📷', members: 210, communityId: null },
    ],
    announcements: [
      { id: 'ann1', title: 'Mid-Semester Examination Schedule Released', body: 'The schedule for mid-semester examinations has been uploaded to the ERP portal. Students must carry their hall tickets. All exams will be held in the designated examination halls as per the timetable.', date: 'Jul 06, 2026', author: 'Examination Cell', isOfficial: true, isPinned: true },
      { id: 'ann2', title: 'Internship & Placement Portal Now Live for 2026 Batch', body: 'Final year students can now register on the placement portal. Ensure your resume is updated and all required documents are uploaded before July 15th. Multiple companies will begin their pre-placement talks from July 20th.', date: 'Jul 04, 2026', author: 'Placement Cell', isOfficial: true, isPinned: true },
      { id: 'ann3', title: 'Tech Fest 2026 — Registrations Open', body: 'GLA Tech Fest 2026 registrations are now open for all students. Early bird registrations get priority for workshops. Visit the Tech Club stall in Block C for offline registration.', date: 'Jul 02, 2026', author: 'GLA Tech Club', isOfficial: false, isPinned: false },
    ],
    resources: [
      { id: 'res1', label: 'Timetable', icon: '📅', url: 'https://gla.ac.in/timetable' },
      { id: 'res2', label: 'Academic Calendar', icon: '🗓️', url: 'https://gla.ac.in/calendar' },
      { id: 'res3', label: 'Exam Notices', icon: '📋', url: 'https://gla.ac.in/notices' },
      { id: 'res4', label: 'Documents', icon: '📄', url: 'https://gla.ac.in/documents' },
      { id: 'res5', label: 'Placement Updates', icon: '💼', url: 'https://gla.ac.in/placements' },
      { id: 'res6', label: 'Exam Info', icon: '📝', url: 'https://gla.ac.in/exams' },
    ],
    importantDates: [
      { label: 'Mid-Sem Exams Begin', date: 'Jul 15, 2026', highlight: true },
      { label: 'Last Day to Add/Drop Course', date: 'Jul 10, 2026', highlight: false },
      { label: 'Tech Fest 2026', date: 'Oct 15, 2026', highlight: false },
      { label: 'Placement Drive Starts', date: 'Nov 02, 2026', highlight: false },
      { label: 'End-Sem Exams', date: 'Dec 01, 2026', highlight: false },
    ],
    trendingDiscussions: [
      { id: 'td1', title: 'Best resources for DSA prep before placements?', replies: 34 },
      { id: 'td2', title: 'Anyone else facing ERP portal issues this week?', replies: 21 },
      { id: 'td3', title: 'Tech Fest project idea brainstorm thread', replies: 18 },
      { id: 'td4', title: 'Which electives are worth taking in 3rd year?', replies: 12 },
    ],
    categories: ['university'],
    tagColor: '#1D4ED8',
    isUniversity: true,
    domain: '@gla.ac.in',
    coverImage: 'https://images.unsplash.com/photo-1562774053-701939374585?w=1200&h=400&fit=crop',
    events: [
      { id: 'e1', title: 'Tech Fest 2026', date: 'Oct 15', time: '10:00 AM', location: 'Main Auditorium', desc: 'Annual technical festival featuring hackathons, workshops, and guest lectures.', image: 'https://images.unsplash.com/photo-1540575467063-178a50c2df87?w=800&q=80', organizer: 'GLA Tech Club', registrationLink: 'https://example.com/register' },
      { id: 'e2', title: 'Campus Recruitment Drive', date: 'Nov 02', time: '9:00 AM', location: 'Placement Block', desc: 'Pre-placement talks and initial screening rounds for final year students.', image: 'https://images.unsplash.com/photo-1515169067868-5387ec356754?w=800&q=80', organizer: 'Placement Cell', registrationLink: 'https://example.com/register' },
      { id: 'e3', title: 'Alumni Meet', date: 'Dec 10', time: '4:00 PM', location: 'University Ground', desc: 'Connect with GLA alumni from various industries.', image: 'https://images.unsplash.com/photo-1523580494112-071d16940a43?w=800&q=80', organizer: 'Alumni Association', registrationLink: 'https://example.com/register' }
    ],
    memberList: [
      { name: 'Rahul S.', avatar: 'R', role: 'Student Council', admin: true, branch: 'B.Tech CS', year: '4th Year', online: true },
      { name: 'Priya K.', avatar: 'P', role: 'Student', branch: 'B.Tech CS', year: '3rd Year', online: true },
      { name: 'Amit V.', avatar: 'A', role: 'Student', branch: 'B.Tech IT', year: '2nd Year', online: false },
      { name: 'Neha G.', avatar: 'N', role: 'Student', branch: 'BCA', year: '1st Year', online: true },
      { name: 'Vikram M.', avatar: 'V', role: 'Student', branch: 'MCA', year: '2nd Year', online: false },
      { name: 'Sneha T.', avatar: 'S', role: 'Student', branch: 'B.Tech EE', year: '3rd Year', online: true },
      { name: 'Rohan D.', avatar: 'R', role: 'Student', branch: 'BBA', year: '2nd Year', online: false },
    ],
    posts: [
      { id: 'c_gla_1', authorId: 'u1', time: '2 hours ago', text: 'Does anyone have the notes for Data Structures unit 3? 📚', likes: 12, comments: 4 },
      { id: 'c_gla_2', authorId: 'u2', time: '5 hours ago', text: 'The new cafeteria menu is finally out! The cold coffee is actually good now.', likes: 45, comments: 18 },
      { id: 'c_gla_3', authorId: 'u3', time: 'Yesterday', text: 'Reminder: The submission deadline for the web dev assignment is tomorrow at 11:59 PM. Don\'t wait until the last minute!', likes: 89, comments: 5 },
      { id: 'c_gla_4', authorId: 'u4', time: '2 days ago', text: 'Looking for teammates for the upcoming Hackathon. Need someone good with React.', likes: 34, comments: 12 },
    ]
  },
  iitdelhi: {
    id: 'iitdelhi',
    name: 'IIT Delhi',
    avatar: '/images/iitd-logo.png',
    color: 'linear-gradient(135deg, #0f766e, #0d9488)',
    desc: 'The official community for Indian Institute of Technology Delhi. Discuss research, campus life, and events.',
    members: 5800,
    online: 1210,
    established: '1961',
    location: 'Hauz Khas, New Delhi, India',
    website: 'https://home.iitd.ac.in',
    collegeType: 'Government (IIT)',
    affiliatedUniversity: 'Institute of National Importance',
    contactEmail: 'webmaster@admin.iitd.ac.in',
    departments: [
      { id: 'iitd_dep_cs', name: 'Computer Science', icon: '💻', members: 1800, communityId: 'iitd_devs' },
      { id: 'iitd_dep_robo', name: 'Robotics Club', icon: '🤖', members: 950, communityId: 'iitd_robotics' },
      { id: 'iitd_dep_elec', name: 'Electronics Club', icon: '⚡', members: 720, communityId: null },
      { id: 'iitd_dep_chem', name: 'Chemical Eng.', icon: '🧪', members: 610, communityId: null },
      { id: 'iitd_dep_arts', name: 'Fine Arts Society', icon: '🎨', members: 450, communityId: null },
      { id: 'iitd_dep_quiz', name: 'Quiz Club', icon: '🧠', members: 390, communityId: null },
    ],
    announcements: [
      { id: 'iitd_ann1', title: 'PhD Admissions 2026 — Applications Open', body: 'Applications for PhD programs (2026-27 session) are now open. Eligible candidates with valid GATE / UGC-NET scores are encouraged to apply through the official portal before August 10th, 2026.', date: 'Jul 05, 2026', author: 'Academic Office', isOfficial: true, isPinned: true },
      { id: 'iitd_ann2', title: 'Tryst 2026 — Theme Announcement & Team Registration', body: "This year's Tryst theme is 'Resilience & Innovation'. Teams of 2–5 members can register through the Tryst portal. Workshops on robotics, AI, and sustainability are being planned.", date: 'Jul 03, 2026', author: 'CAIC', isOfficial: false, isPinned: true },
      { id: 'iitd_ann3', title: 'Inter-IIT Sports Meet — Selection Trials', body: 'Trials for the Inter-IIT Sports Meet will be held from July 12–15 at the Sports Complex. Students interested in participating must register with their respective Sports Society by July 10th.', date: 'Jul 01, 2026', author: 'Sports Council', isOfficial: true, isPinned: false },
    ],
    resources: [
      { id: 'iitd_res1', label: 'Timetable', icon: '📅', url: 'https://home.iitd.ac.in/timetable' },
      { id: 'iitd_res2', label: 'Academic Calendar', icon: '🗓️', url: 'https://home.iitd.ac.in/calendar' },
      { id: 'iitd_res3', label: 'Course Registration', icon: '📋', url: 'https://home.iitd.ac.in/courses' },
      { id: 'iitd_res4', label: 'Library Portal', icon: '📚', url: 'https://library.iitd.ac.in' },
      { id: 'iitd_res5', label: 'Internship Portal', icon: '💼', url: 'https://home.iitd.ac.in/internships' },
      { id: 'iitd_res6', label: 'Hostel Mess Menu', icon: '🍽️', url: 'https://home.iitd.ac.in/hostel' },
    ],
    importantDates: [
      { label: 'PhD Application Deadline', date: 'Aug 10, 2026', highlight: true },
      { label: 'Sports Meet Trials', date: 'Jul 12, 2026', highlight: true },
      { label: 'Tryst 2026 Begins', date: 'Oct 22, 2026', highlight: false },
      { label: 'Rendezvous 2026', date: 'Dec 05, 2026', highlight: false },
      { label: 'Inter-IIT Tech Meet', date: 'Jan 18, 2027', highlight: false },
    ],
    trendingDiscussions: [
      { id: 'iitd_td1', title: 'GPU cluster CUDA OOM errors — solutions?', replies: 28 },
      { id: 'iitd_td2', title: 'Tryst 2026 theme ideas & team finding', replies: 45 },
      { id: 'iitd_td3', title: 'Best hostel messes ranked by students', replies: 61 },
      { id: 'iitd_td4', title: 'Research internship experiences — share yours', replies: 33 },
    ],
    categories: ['university'],
    tagColor: '#0f766e',
    isUniversity: true,
    domain: '@iitd.ac.in',
    coverImage: 'https://images.unsplash.com/photo-1523050854058-8df90110c9f1?w=1200&h=400&fit=crop',
    events: [
      { id: 'e4', title: 'Tryst 2026', date: 'Oct 22', time: '09:30 AM', location: 'Dogra Hall', desc: 'North India\'s largest science and technology festival.', image: 'https://images.unsplash.com/photo-1531482615713-2afd69097998?w=800&q=80', organizer: 'Co-Curricular & Academic Interaction Council', registrationLink: 'https://example.com/register' },
      { id: 'e5', title: 'Rendezvous 2026', date: 'Dec 05', time: '06:00 PM', location: 'OAT', desc: 'Annual cultural festival with star nights, drama, and musical battles.', image: 'https://images.unsplash.com/photo-1459749411175-04bf5292ceea?w=800&q=80', organizer: 'Board for Recreational and Creative Activities', registrationLink: 'https://example.com/register' },
      { id: 'e6', title: 'Inter-IIT Tech Meet', date: 'Jan 18', time: '10:00 AM', location: 'LHC', desc: 'The ultimate showcase of technical solutions and innovations.', image: 'https://images.unsplash.com/photo-1519389950473-47ba0277781c?w=800&q=80', organizer: 'Tech Board IITD', registrationLink: 'https://example.com/register' }
    ],
    memberList: [
      { name: 'Sammy D.', avatar: 'S', role: 'Event Lead', admin: true, branch: 'B.Tech CS', year: '1st Year', online: true },
      { name: 'Marcus R.', avatar: 'M', role: 'Student', branch: 'B.Tech IT', year: '3rd Year', online: true },
      { name: 'Sarah J.', avatar: 'S', role: 'Researcher', branch: 'M.Tech CSE', year: '2nd Year', online: true },
      { name: 'Chris B.', avatar: 'C', role: 'Student', branch: 'B.Tech ME', year: '4th Year', online: false },
    ],
    posts: [
      { id: 'c_iitd_1', authorId: 'u8', time: '3 hours ago', text: 'Struggling with the GPU cluster setup for ML training. Anyone else getting CUDA out of memory errors?', likes: 23, comments: 7 },
      { id: 'c_iitd_2', authorId: 'u2', time: 'Yesterday', text: 'Tryst 2026 project submissions are now open. Get your teams registered!', likes: 54, comments: 11 },
      { id: 'c_iitd_3', authorId: 'u4', time: '2 days ago', text: 'Who is up for a late-night night-out to SDA market for paranthas? 🫓☕', likes: 72, comments: 20 },
    ]
  },
  gla_coding: {
    id: 'gla_coding',
    name: 'GLA Coding Club',
    avatar: '💻',
    color: 'linear-gradient(135deg, #2563eb, #3b82f6)',
    desc: 'The official programming club of GLA University. Code, compile, conquer!',
    members: 1200,
    online: 43,
    created: 'Sept 2023',
    categories: ['technology'],
    tagColor: '#2563eb',
    collegeId: 'gla',
    coverTheme: 'coding',
    discussionsToday: 15,
    eventsThisWeek: 3,
    newMembersThisWeek: 45,
    trending: false,
    growth: '+45 this week',
    categoryLabel: 'Coding',
    memberAvatars: ['R', 'N'],
    memberList: [
      { name: 'Rahul S.', avatar: 'R', role: 'Lead Developer' },
      { name: 'Neha G.', avatar: 'N', role: 'Member' }
    ],
    posts: [
      { id: 'gla_coding_1', authorId: 'u1', time: '3 hours ago', text: 'Starting our Weekly LeetCode challenge tonight at 8 PM! Don\'t miss out.', likes: 15, comments: 2 }
    ]
  },
  gla_sports: {
    id: 'gla_sports',
    name: 'GLA Sports Club',
    avatar: '⚽',
    color: 'linear-gradient(135deg, #dc2626, #ef4444)',
    desc: 'Where athletes meet. Join for football, cricket, badminton updates.',
    members: 850,
    online: 19,
    created: 'Oct 2023',
    categories: ['sports'],
    tagColor: '#dc2626',
    collegeId: 'gla',
    coverTheme: 'sports',
    discussionsToday: 8,
    eventsThisWeek: 5,
    newMembersThisWeek: 22,
    trending: false,
    growth: '+22 this week',
    categoryLabel: 'Sports',
    memberAvatars: ['A', 'S'],
    memberList: [
      { name: 'Amit V.', avatar: 'A', role: 'Captain' },
      { name: 'Sneha T.', avatar: 'S', role: 'Member' }
    ],
    posts: [
      { id: 'gla_sports_1', authorId: 'u3', time: '1 day ago', text: 'Inter-college cricket match tomorrow. Support our team at the main ground!', likes: 28, comments: 5 }
    ]
  },
  iitd_robotics: {
    id: 'iitd_robotics',
    name: 'IITD Robotics Club',
    avatar: '🤖',
    color: 'linear-gradient(135deg, #059669, #10b981)',
    desc: 'Building autonomous agents, drones, and bots at IIT Delhi.',
    members: 950,
    online: 35,
    created: 'Aug 2022',
    categories: ['technology', 'science'],
    tagColor: '#059669',
    collegeId: 'iitdelhi',
    coverTheme: 'ai',
    discussionsToday: 12,
    eventsThisWeek: 2,
    newMembersThisWeek: 31,
    trending: false,
    growth: '+31 this week',
    categoryLabel: 'Robotics',
    memberAvatars: ['M', 'S'],
    memberList: [
      { name: 'Marcus R.', avatar: 'M', role: 'Robotics Lead' },
      { name: 'Sarah J.', avatar: 'S', role: 'Member' }
    ],
    posts: [
      { id: 'iitd_robotics_1', authorId: 'u2', time: '2 hours ago', text: 'Testing our new line-follower algorithm on the arena. Come watch in room 402!', likes: 42, comments: 8 }
    ]
  },
  iitd_devs: {
    id: 'iitd_devs',
    name: 'IITD Developer Guild',
    avatar: '⚡',
    color: 'linear-gradient(135deg, #7c3aed, #8b5cf6)',
    desc: 'Open-source contributors and web developers at IIT Delhi.',
    members: 1100,
    online: 52,
    created: 'Sept 2022',
    categories: ['technology'],
    tagColor: '#7c3aed',
    collegeId: 'iitdelhi',
    coverTheme: 'coding',
    discussionsToday: 18,
    eventsThisWeek: 4,
    newMembersThisWeek: 67,
    trending: false,
    growth: '+67 this week',
    categoryLabel: 'Devs',
    memberAvatars: ['S', 'C'],
    memberList: [
      { name: 'Sammy D.', avatar: 'S', role: 'Founder' },
      { name: 'Chris B.', avatar: 'C', role: 'Contributor' }
    ],
    posts: [
      { id: 'iitd_devs_1', authorId: 'u4', time: '1 day ago', text: 'We are planning an open source hackathon next month. Submit your theme ideas!', likes: 37, comments: 14 }
    ]
  }
};

export const categoriesList = [
  { id: 'all', label: 'All' },
  { id: 'colleges', label: 'Colleges' },
  { id: 'design', label: 'Design' },
  { id: 'ai', label: 'AI' },
  { id: 'startup', label: 'Startup' },
  { id: 'coding', label: 'Coding' },
  { id: 'gaming', label: 'Gaming' },
  { id: 'hackathons', label: 'Hackathons' },
  { id: 'sports', label: 'Sports' },
  { id: 'marketing', label: 'Marketing' },
  { id: 'career', label: 'Career' },
  { id: 'product', label: 'Product' },
  { id: 'music', label: 'Music' },
  { id: 'photography', label: 'Photography' },
  { id: 'science', label: 'Science' },
  { id: 'books', label: 'Books' },
  { id: 'art', label: 'Art' },
  { id: 'finance', label: 'Finance' },
  { id: 'health', label: 'Health & Fitness' },
  { id: 'blockchain', label: 'Blockchain & Web3' },
  { id: 'language', label: 'Language Learning' },
  { id: 'food', label: 'Food & Drink' },
  { id: 'fashion', label: 'Fashion' },
  { id: 'film', label: 'Film & TV' },
  { id: 'travel', label: 'Travel & Outdoors' },
  { id: 'education', label: 'Education' },
  { id: 'pets', label: 'Pets & Animals' },
  { id: 'other', label: 'Other' },
];
